...
def main():
    input_folder_path = "./test/images/input"
    output_folder_path = "./test/images/output"
    all_images = load_images_from_folder(input_folder_path)
    print('Total Images: ',len(all_images))
    HEIGHT = 32
    WIDTH = 32

    net = cv2.dnn.readNet("./model/yolov3_training_last.weights", "./model/yolov3_training.cfg")

    classes = []
    with open("./model/signs_names.txt", "r") as f:
    ...
    classification_model = load_model('./model/traffic.h5') #load mask detection model
    ...
    with open("./model/signs_classes.txt", "r") as f:
        ...
        for i in range(len(all_images)):
            ...
            cv2.imwrite(str(output_folder_path)+'/'+ str(i+1) + '.jpg', img)
